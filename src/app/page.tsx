...
export default function Home() {
  ...
  const moveTask = (dragPosition: number, hoverPosition: number) => {
    ...
    const draggedTaskIndex = updatedToDo.findIndex(task => task.position === dragPosition);
    const [draggedTask] = updatedToDo.splice(draggedTaskIndex, 1);  
  
    const hoverTaskIndex = updatedToDo.findIndex(task => task.position === hoverPosition);
    updatedToDo.splice(hoverTaskIndex, 0, draggedTask);  
  ...
  .map((item) => (
    ...
    />
        ...
        <div className="control has-text-centered"> {/* Centralize the button */}
